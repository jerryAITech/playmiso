    ...
    async function loadData() {
      try {
        ...
        if (catRes.ok && prodRes.ok) {
          ...
          const p: ProductType = await prodRes.json();
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <h1 className="text-2xl sm:text-3xl font-black text-slate-900">
            Edit Toy Product & SEO
          </h1>
          <p className="text-xs text-slate-500 mt-1">
            Update pricing, stock, images, safety info, and Google SEO snippet tags.
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white border border-slate-200 rounded-3xl p-6 sm:p-8 shadow-sm space-y-6">
        ...
        <div className="space-y-4 pt-2">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div>
              <label className="block text-xs font-bold text-slate-700 mb-1">
                Stock Count <span className="text-toy-red">*</span>
              </label>
              <input
                type="number"
                required
          ...
          {calculatedDiscount > 0 && (
            <div className="bg-emerald-50 text-emerald-800 text-xs font-bold p-3 rounded-2xl border border-emerald-200 flex items-center gap-2">
              ...
              <span>
                Discount Savings: <strong>{calculatedDiscount}% OFF</strong> (Customer saves ₹
                {mrpNum - priceNum})
              </span>
            </div>
          )}
        ...
        <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-200">
          ...
          >
            {submitting ? (
              ...
              <>
                <span>Save Changes & Update SEO</span>
